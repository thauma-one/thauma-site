 ...
 */
import { parseFeed, parseChannelTitle, feedUrl, CHANNEL_ID_RE } from "./youtube.js";
...
export async function syncChannel(db, channel, { fetchImpl = fetch, now } = {}) {
  const stamp = now || new Date().toISOString();
  const partner_id = channel.partner_id ?? null;
  const channel_id = channel.channel_id;

  if (!CHANNEL_ID_RE.test(channel_id || "")) {
    await db.query("video_channel_failed", {
      partner_id, now: stamp, error: "That channel id is not the right shape.",
    });
    return { ok: false, error: "bad channel id", count: 0 };
  }

  let body;
  try {
    const res = await fetchImpl(feedUrl(channel_id), {
      headers: { Accept: "application/atom+xml, application/xml" },
    });
    if (!res.ok) {
      /* 404 is the one worth naming: it almost always means the channel was
      ...
      await db.query("video_channel_failed", { partner_id, now: stamp, error: why });
  ...
  /* Safe now: the response was a feed, so an empty one means an empty
     channel. Everything the run just touched carries `stamp`. */
  await db.query("videos_prune", { channel_id, now: stamp });
  await db.query("video_channel_synced", { partner_id, now: stamp });

  return { ok: true, count: videos.length, title: parseChannelTitle(body) };
}
...
export async function syncAll(db, opts = {}) {
  const channels = await db.query("video_channels_all", {});
  const results = [];
  for (const c of channels) {
    try {
      results.push({ partner_id: c.partner_id, ...(await syncChannel(db, c, opts)) });
    } catch (err) {
      /* syncChannel records its own failures; reaching here means something
         unforeseen, and the next channel still deserves its turn. */
